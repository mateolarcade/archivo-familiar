(function () {
  ...
  function renderNotFound() {
    document.title = "REC | Contenido no encontrado";
    playerContent.innerHTML = "<section class=\"not-found\"><p class=\"eyebrow\">Contenido no disponible</p><h1>No encontramos ese video</h1><p>El enlace puede estar incompleto o el ID no existe en el catalogo.</p><a class=\"primary-link\" href=\"index.html?catalog=1\">Volver al catalogo</a></section>";
  }

  function renderDetail(item) {
    const category = item.category || item.type || "Video REC";
    const people = Array.isArray(item.people) ? item.people.join(", ") : item.people || "No especificado";
    document.title = "REC | " + item.title;
    playerContent.innerHTML = "<section class=\"detail-layout\"><aside class=\"detail-poster\"><img src=\"" + escapeAttribute(item.poster) + "\" alt=\"Portada de " + escapeAttribute(item.title) + "\"></aside><article class=\"detail-panel\"><h1>" + escapeHtml(item.title) + "</h1><p class=\"mobile-location\"><span>Lugar</span><strong>" + escapeHtml(item.location || "No especificado") + "</strong></p><div class=\"detail-meta-grid\"><div class=\"detail-meta-year\"><span>A&ntilde;o</span><strong>" + escapeHtml(item.year) + "</strong></div><div class=\"detail-meta-location\"><span>Lugar</span><strong>" + escapeHtml(item.location || "No especificado") + "</strong></div><div class=\"detail-meta-duration\"><span>Duracion</span><strong>" + escapeHtml(item.duration) + "</strong></div><div class=\"detail-meta-category\"><span>Categoria</span><strong>" + escapeHtml(category) + "</strong></div></div><div class=\"people-block description-block\"><span>Descripcion</span><p>" + escapeHtml(item.description) + "</p></div><p class=\"detail-description\"><strong>Personas que aparecen:</strong> " + escapeHtml(people) + "</p><p class=\"mobile-detail-description\">" + escapeHtml(item.description) + "</p><button id=\"play-button\" class=\"primary-link play-button\" type=\"button\">Ver video</button></article></section><section class=\"watch-layout\" aria-label=\"Reproductor de video\"><div id=\"video-frame\" class=\"video-frame\"></div>" + renderAccessCard() + "</section>";
