(function () {
  ...
  let attempts = 0;
  function shouldOpenCatalogDirectly() { return new URLSearchParams(window.location.search).get("view") === "catalog"; }
  ...
  function startExperience() { if (isLoggedIn() || shouldOpenCatalogDirectly()) { if (config.rememberLoginInSession) sessionStorage.setItem("archivoFamiliarAuth", "ok"); openCatalog(); return; } showOnly(introScreen); window.setTimeout(openLogin, 3300); }
